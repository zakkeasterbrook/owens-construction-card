document.addEventListener('DOMContentLoaded', () => {
    ...
    // Set the initial scale of the AR model to 25%
    modelViewer.addEventListener('model-visibility', () => {
        modelViewer.style.transform = 'scale(0.25)';
    });
    ...
    function showCaptions(text, audio, duration) {
        ...
        const updateCaptions = () => {
            if (index < words.length) {
                ...
                const segmentLength = Math.min(4, words.length - index);
                const segment = words.slice(index, index + segmentLength).join(' ');
                caption.textContent = segment;
                index += segmentLength;
                setTimeout(updateCaptions, intervalTime);
            } else {
                // Hide caption when done
